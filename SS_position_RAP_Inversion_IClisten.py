...
N_mean = 2.31                   # Geoid height at lat/lon
IC_lat = 22.739153
IC_lon = -158.0061254
IC_z = -4728.17 - N_mean
...
RAP_data = sio.loadmat('RAP_test_IC_flat_earth.mat')['RAP_test'][0][0]
...
max_lat = Geodesic.WGS84.Direct(IC_lat, IC_lon, 0, g_bound)['lat2']
max_lon = Geodesic.WGS84.Direct(IC_lat, IC_lon, 90, g_bound)['lon2']
min_lat = Geodesic.WGS84.Direct(IC_lat, IC_lon, 180, g_bound)['lat2']
min_lon = Geodesic.WGS84.Direct(IC_lat, IC_lon, 270, g_bound)['lon2']
...
for lat, lon in zip(tx_lat, tx_lon):
    x_dist.append(Geodesic.WGS84.Inverse(lat, lon, IC_lat, IC_lon)['s12'])
    azmth.append(Geodesic.WGS84.Inverse(lat, lon, IC_lat, IC_lon)['azi1'])
...
# for t_num in range(10):
    ...
    ray = Ray_Tracing(x_dist[t_num], z_tx[t_num], tx_lat[t_num], azmth[t_num], IC_lat, IC_lon, IC_z)
    ...
    pix_dist, pix_num = pixel_distance(tx_lat[t_num], tx_lon[t_num], IC_lat, IC_lon, x_ticks, y_ticks)
...
py_mat_diff = t_diff1[:num_tx] - t_diff2                # tt estimate diff between matlab and python code
...
plt.title('TT Perturbations (icListen)')
plt.xlabel('Range (km)')
plt.ylabel('(ms)')
plt.grid(True)
plt.show()
...
# ax.legend((mat_t_plt, py_t_plt), ('Matlab', 'Python'))
# plt.title('Travel times (icListen)')
# plt.xlabel('Range (km)')
# plt.ylabel('(ms)')
# plt.grid(True)
# plt.show()
...
plt.title('Difference between Matlab and Python TT (icListen)')
plt.xlabel('Range (km)')
plt.ylabel('(s)')
plt.grid(True)
